...
//
// Every effect below is an engine built-in declared in r_efx.h. They are all
// model-free (no sprite/model index needed) so none can silently fail and
// none can produce the flat black squares that Alpha-blended sprites caused.
//
// Layers, in order: flash -> fire -> spark -> ground debris -> dust -> light.
...
void EV_CreateExplo(event_args_s *args)
{
	...
	{
		// nothing solid below (air burst) -> use a spot under the blast
		groundPos[0] = org[0];
		groundPos[1] = org[1];
	...
	// =======================================================================
	//  4. GROUND DEBRIS layer (classic little specks)
	// =======================================================================
	for( int i = 0; i < 6; i++ )
	{
	...
	// =======================================================================
	//  6. DYNAMIC LIGHT layer
	//     decay/minlight are set so the light fades smoothly instead of
	//     cutting off hard at ->die.
	// =======================================================================
